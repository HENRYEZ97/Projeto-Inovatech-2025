...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full w-64 bg-white shadow-xl transform ${open ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 z-50`}>
      <div className="p-6 border-b bg-gradient-to-br from-sky-200 to-cyan-200 flex justify-between items-center">
        ...
        <button onClick={onClose} className="text-white font-bold cursor-pointer hover:bg-blue-500 rounded-br-lg">×</button>
      </div>
      <ul className="p-4">
        {locais.map((local) => (
